            ...
            using (var scope = _scopeFactory.CreateScope())
            {
                ...
                switch (executer.Status)
                {
                    ...
                    case 0:
                        ...
                        _logger.LogInformation($"{executer.Timer.WorkerName} 任务开始通知");
                        break;
                    // 任务执行之前通知
                    case 1:
                        ...
                        _logger.LogInformation($"{executer.Timer.WorkerName} 执行之前通知");
                        break;
                    // 执行成功通知
                    case 2:
                        _logger.LogInformation($"{executer.Timer.WorkerName} 执行成功通知");
                        break;
                    // 任务执行失败通知
                    case 3:
                    ...
                    case -1:
                        ...
                        dbTimer.RunErrorNumber = 0;
                        await _repository.UpdateIncludeNowAsync(dbTimer, new[] { nameof(SysTimerEntity.RunErrorNumber), nameof(SysTimerEntity.RunNumber), nameof(SysTimerEntity.Started), nameof(SysTimerEntity.UpdatedTime) });
                        _logger.LogWarning($"{executer.Timer.WorkerName} 执行停止通知");
                        break;
                    // 任务执行取消通知
                    case -2:
                        _logger.LogWarning($"{executer.Timer.WorkerName} 执行取消通知");
                        break;
                    default:
                        break;
                }
