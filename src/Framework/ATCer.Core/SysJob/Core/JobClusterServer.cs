...
public class JobClusterServer : IJobClusterServer
{
    ...
    public JobClusterServer(IRepository<SysJobCluster> sysJobClusterRep)
    {
    ...
    public async void Start(JobClusterContext context)
    {
        ...
        {
            await _sysJobClusterRep.Where(x => x.ClusterId == context.ClusterId)
                                   .ExecuteUpdateAsync(x => x.SetProperty(x => x.Status, ClusterStatus.Waiting));
        }
        else
        {
            await _sysJobClusterRep.InsertNowAsync(new SysJobCluster { ClusterId = context.ClusterId, Status = ClusterStatus.Waiting});
        }
    }
    ...
    public async void Stop(JobClusterContext context)
    {
        ...
        await _sysJobClusterRep.Where(x => x.ClusterId == context.ClusterId)
                               .ExecuteUpdateAsync(x => x.SetProperty(x => x.Status, ClusterStatus.Crashed));
    }
    ...
    private async void WorkNowAsync(string clusterId)
    {
        ...
        await _sysJobClusterRep.Where(x => x.ClusterId == clusterId)
                               .ExecuteUpdateAsync(x => x.SetProperty(x => x.Status, ClusterStatus.Working));
    }
}