...
public class DbJobPersistence : IJobPersistence
{
    ...
    public IEnumerable<SchedulerBuilder> Preload()
    {
        ...
        var jobDetailRep = serviceScope.ServiceProvider.GetService<IRepository<SysJobDetail>>();
        var jobTriggerRep = serviceScope.ServiceProvider.GetService<IRepository<SysJobTrigger>>();

        // 获取内存的作业
        IEnumerable<SchedulerBuilder> memoryJobs = App.EffectiveTypes.ScanToBuilders();

        // 若数据库不存在任何作业，则返回内存作业
        var anyJobDetails = jobDetailRep?.Any();
        if (anyJobDetails.Value == true) return memoryJobs;

        var schedulerBuilders = new List<SchedulerBuilder>();

        // 获取数据库所有作业
        var dbJobs = jobDetailRep.AsQueryable(false).ToList();
        foreach (var dbJob in dbJobs)
        {
            ...
            var dbTriggers = jobTriggerRep.Where(u => u.JobId == dbJob.JobId).ToList()
    ...
    public SchedulerBuilder OnLoading(SchedulerBuilder builder)
    {
        ...
        {
            if (jobTriggerRep.Where(u => u.JobId == jobBuilder.JobId && u.TriggerId == triggerBuilder.TriggerId).Any())
    ...
    {
        using var serviceScope = _serviceProvider.CreateScope();
        var db = serviceScope.ServiceProvider.GetService<IRepository<SysJobDetail>>();

        var jobDetail = context.JobDetail.Adapt<SysJobDetail>();
        if (context.Behavior == PersistenceBehavior.Appended)
        {
            db?.InsertNow(jobDetail);
        }
        else if (context.Behavior == PersistenceBehavior.Updated)
        {
            var lst = db?.Where(x => x.JobId == jobDetail.JobId)
                         .ToList();

            foreach (var item in lst)
            {
                db?.UpdateNow(item.Adapt(jobDetail));
            }
        }
        else if (context.Behavior == PersistenceBehavior.Removed)
        {
            db?.Where(x => x.JobId == jobDetail.JobId).ExecuteDelete();
        }
    }
    ...
    {
        using var serviceScope = _serviceProvider.CreateScope();
        var db = serviceScope.ServiceProvider.GetService<IRepository<SysJobTrigger>>();

        var jobTrigger = context.Trigger.Adapt<SysJobTrigger>();
        if (context.Behavior == PersistenceBehavior.Appended)
        {
            //db.Insertable(jobTrigger).ExecuteCommand();
            db?.InsertNow(jobTrigger);
        }
        else if (context.Behavior == PersistenceBehavior.Updated)
        {
            var lst =db?.Where(x => x.TriggerId == jobTrigger.TriggerId && x.JobId == jobTrigger.JobId)
                        .ToList();

            foreach (var item in lst)
            {
                db?.UpdateNow(item.Adapt(jobTrigger));
            }
        }
        else if (context.Behavior == PersistenceBehavior.Removed)
        {
            db?.Where(x=>x.TriggerId == jobTrigger.TriggerId && x.JobId == jobTrigger.JobId)
               .ExecuteDelete();
        }
    }
}