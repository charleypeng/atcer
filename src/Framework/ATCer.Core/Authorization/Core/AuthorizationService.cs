...
namespace ATCer.Authorization.Core
{
    ...
    public class AuthorizationService : IAuthorizationService
    {
        ...
        public async Task<bool> ChecktContenxtApiEndpoint()
        {
            ...
            //当使用AppToken时不适用
            if(identity.IdentityType != Authentication.Enums.IdentityType.AppToken)
            {
                //clientId 已不可用
        ...
        /// </summary>
        /// <returns></returns>
        private (MyHttpMethod, string) GetContextEndpoint()
        {
            //没有特性的可以通过路由+请求方法查找
            MyHttpMethod method =Enum.Parse< MyHttpMethod>(_httpContextAccessor.HttpContext.Request.Method.ToUpper());
