	...
	def Recive_data(self,):
		...
		while True:
			try:
				...
				if count != 0:
					ss = self.main_engine.read(self.main_engine.in_waiting)
...
SerialCom.Print_Used_Com()
Ret =False #是否创建成功标志

Engine1 = SerialCom("com2",9600,0.5)
if (Ret):
	Engine1.Open_Engine()
	Engine1.Recive_data()
...
'''th1 = threading.Thread(target=fun4,daemon=True)
th2 = threading.Thread(target=fun4,daemon=True)
th1.start()
th2.start()'''

#executor = ThreadPoolExecutor(max_workers=2)
#a1 = executor.submit(fun1)
#a2 = executor.submit(fun2,x="119",y=123,z =9)
