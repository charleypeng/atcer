...
namespace ATCer.HRCenter.Services
{
    ...
    public class SectorService:ATCer.ServiceBase<Sector, SectorDto, int>, ISectorService
    {
        ...
        /// <returns></returns>
        [HttpGet]
        public async Task NewSector()
        {
            var sector = new SectorDto { Code = "C12", Department = Enums.ATCDepartment.ACC, Multiplier = 1.01, Position = Enums.PositionRole.InCommand, PhysicalPosition = "ZXZT" };
            await this.Insert(sector);
        }

        public async Task ImportSector(IList<ImportSectorDto> sector)
        {
