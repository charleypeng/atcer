...
namespace ATCer.UserCenter.Impl.Services
{
    ...
    public class ClientService : ServiceBase<Client, ClientDto, Guid>, IClientService
    {
        ...
        public async Task<TokenOutput> Login(ClientLoginInput input)
        {
            ...
            Client client = await _repository.AsQueryable(false).Where(x => x.Id.Equals(input.ClientId) && x.IsDeleted == false && x.IsLocked == false).FirstOrDefaultAsync();
            if (client == null)
            {
        ...
        /// <returns></returns>
        [AllowAnonymous, IgnoreAudit]
        public async Task<TokenOutput> Login(string secretkey)
        {
            Client client = await _repository.AsQueryable(false).Where(x => x.SecretKey.Equals(secretkey) && x.IsDeleted == false && x.IsLocked == false).FirstOrDefaultAsync();
            if (client == null)
            {
                throw Oops.Bah(ExceptionCode.CLIENT_NO_FIND);
            }
