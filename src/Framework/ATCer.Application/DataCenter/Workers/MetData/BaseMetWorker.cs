...
public abstract class BaseRadarWorker<TDto, TKey> : ICapSubscribe where TDto : BaseDto<string>, new()
{
    /// <summary>
    /// 
    /// </summary>
    ...
    public BaseRadarWorker(IServiceBase<TDto, TKey> client,
                      ILogger logger)
    {
        _client = client;
        _logger = logger;
    ...
    public virtual async Task AddDataAsync(RawMetData data)
    {
        ...
        //insert
        if (result != null)
            _logger.LogInformation($"{typeof(BaseRadarWorker<TDto, TKey>).Name}已入库：datetime{DateTime.Now}:{JsonConvert.SerializeObject(mdata)}");
    }
}
